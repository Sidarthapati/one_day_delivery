...
class AmqpTapConfig {
    ...
    RabbitTemplate rabbitTemplate(RabbitTemplateConfigurer configurer,
        ...
        RabbitTemplate template = new RabbitTemplate() {
            ...
            public void send(String exchange, String routingKey, Message message, CorrelationData correlationData) {
                try {
                    tap.publish(exchange, routingKey, typeOf(message), detailOf(message));
    ...
    SimpleRabbitListenerContainerFactory rabbitListenerContainerFactory(
        ...
        factory.setAfterReceivePostProcessors(message -> {
            try {
                tap.consume(message.getMessageProperties().getConsumerQueue(), typeOf(message), detailOf(message));
            } catch (RuntimeException ignore) {
    ...
    private static final com.fasterxml.jackson.databind.ObjectMapper SNIFFER =
            new com.fasterxml.jackson.databind.ObjectMapper();

    /**
     * Short human hint from the JSON payload for the feed line: shipment ref, state transition
     * (from→to), or DA event type — whichever fields the event carries. Payloads are snake_case (the
     * app's global Jackson strategy). Never throws.
     */
    private static String detailOf(Message message) {
        try {
            com.fasterxml.jackson.databind.JsonNode n = SNIFFER.readTree(message.getBody());
            StringBuilder d = new StringBuilder();
            if (n.hasNonNull("shipment_ref")) d.append(n.get("shipment_ref").asText());
            String from = n.path("from_state").asText(null);
            String to = n.path("to_state").asText(null);
            if (to != null) d.append(d.isEmpty() ? "" : " ").append(from != null ? from + "→" : "→").append(to);
            String evt = n.path("event_type").asText(null);
            if (evt != null && d.isEmpty()) d.append(evt);
            if (d.isEmpty()) {
                String parcel = n.path("parcel_id").asText(null);
                if (parcel != null) d.append("parcel ").append(parcel, 0, Math.min(8, parcel.length()));
            }
            return d.isEmpty() ? null : d.toString();
        } catch (Exception e) {
            return null;
        }
    }
