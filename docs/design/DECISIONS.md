...
### M4-D-006 — Shipment State Machine (Draft)
...
Failure branches: `PICKUP_FAILED`, `DELIVERY_FAILED`, `RTO_INITIATED`, `RTO_IN_TRANSIT`, `RTO_COMPLETED`, `CANCELLED`

**What needs to be decided:**  
Full sign-off from ops on every state and transition before implementation. States may be added (e.g. `CUSTOMS_HOLD`) or collapsed (e.g. `DEPARTED` + `IN_TRANSIT`).
