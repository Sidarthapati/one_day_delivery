...
- **✅ FIXED (2026-06-14):** all three implemented in `RoutePlanningServiceImpl.recommendVanCount`:
  1. **(A) Structural-infeasibility bail** — before any solve, scan each vertex: if `max(deliver,collect) > capacity` OR its solo hub round-trip > cycle, return immediately with a human reason. Now returns `VanRecommendation(count, note)` where `note` is non-null only on a structural fallback.
  2. **(B) Fast probe** — new `VanRouteSolver.probe(...)`; OR-Tools override sets `setSolutionLimit(1)` + a short `routing.solver.probe-time-limit-seconds` (=3) so it stops at the **first feasible** solution instead of optimising.
  3. **(C) Binary search** over `k ∈ [lowerBound, vertexCount]` (feasibility is monotonic in van count) instead of the linear scan.
  - **Result:** Delhi cycle-180 case **~8 min → ~23s**; the bail fires instantly and names the offending vertex. With drop-and-flag (item #8) `recommendVanCount` runs on the **served** set (`reduce()` strips deferred vertices) so it returns a real number, not the `vertexCount` fallback.
  - **Files:** `VanRouteSolver` (probe default), `OrToolsVanRouteSolver` (`solveInternal(feasibilityOnly)`), `VanRouteSolverSelector`, `RoutingProperties.Solver.probeTimeLimitSeconds`, `RoutePlanningServiceImpl`.
- [x] `recommendVanCount`: structural bail + binary search + probe — DONE on demo branch
- [ ] ported to `f-m6-design`
...
### 7. Per-van cadence — kill the global slowest-van throttle (CARRY OVER) ✅
- **Symptom:** the assembler clocked **every** van to one global cadence = `max(allSpans)` and one global `n_loops`, so a single slow van dragged the whole city to 2 sweeps/day even when most vans could sweep 4–5×.
- **Why the uniformity was wrong (not just slow):** it was a modeling shortcut, **not** an operational constraint. **M5 (dispatch) reads each van's stamped meeting times into the DA's priority queue**, so the fleet does not need a shared cadence. Design rationale written up in **`docs/M6/M6-INFEASIBLE-VERTICES.md` §3a**.
- **Fix applied (`RoutePlanAssembler`):** each van now repeats its **own** loop at cadence `max(itsSpan, cronFreeze)` and sweeps `window / itsCadence` times. Because per-loop quantities depend on a van's loop count, each vertex's **daily** demand is re-split by the serving van's own loops and load re-walked (day-totals stay exact). Assembler signature changed: **dropped the `nodes` param, added `Map<UUID,double[]> dailyByVertex`**; `RoutePlanningServiceImpl` computes `dailyDemandByVertex(...)` once and feeds both `buildNodes` and the assembler. Plan `nLoops` = max van loops, `realisedCycleMinutes` = max span. `RoutePlanAssemblerTest` updated (now passes `dailyByVertex`, asserts recomputed load).
- **Result (Delhi, all served):** uniform 2 loops → **2–4 spread**. The global `n_loops` fixed-point in `plan()` is kept but now only feeds the solver's capacity check; the assembler re-periodises per van.
- [ ] ported to `f-m6-design`

### 8. Drop-and-flag — defer far corners instead of throttling the fleet (CARRY OVER) ✅
- **Symptom:** a vertex whose **solo** hub round-trip exceeds the cycle pins the OR-Tools makespan (objective = `travel + 100×makespan`), so extra vans can't lower it — 25 vans still used 15, all at 2 sweeps.
- **Fix applied:** the shipping solve now runs against the **cycle target** (not the window) and makes each vertex **optional** via an OR-Tools disjunction with a prohibitive penalty (`1e9`, far above any seconds cost). The solver serves what it can and **defers** only vertices unreachable within the cycle. Removing them drops the makespan floor → the solver uses the whole fleet, every loop fits the target.
  - `SolveResult` gained `List<UUID> droppedVertexIds` (back-compat 3-arg constructor → empty).
  - `VanRouteSolver.solve(..., boolean allowDrops)` (default ignores drops); `OrToolsVanRouteSolver.solveInternal(feasibilityOnly, allowDrops)` adds the disjunctions + collects unvisited vertices; `VanRouteSolverSelector` overrides the 5-arg.
  - `RoutePlanningServiceImpl`: drop-mode wiring (`solveCycleMax = dropMode ? cycleTimeMax : window`), `reduce()` strips deferred vertices before `recommendVanCount`, `droppedNote()`/`combineNotes()`/`serializeIds()`.
  - **Config:** `routing.solver.drop-infeasible-vertices` (default **true**; in app yaml + `RoutingProperties.Solver`). `false` = old serve-everything-on-relaxed-cycle behaviour.
- **Tradeoff:** coverage vs sweeps, dialed by the cycle target (lower = more sweeps, more drops). Delhi cycle-180: 15 vans/2 loops/277min → **25 vans/4–5 loops/163min/71 served/20 deferred**. Full writeup: **`docs/M6/M6-INFEASIBLE-VERTICES.md`**.
- **Tests:** +2 in `OrToolsVanRouteSolverTest` (`dropAndFlagDefersFarVertexAndServesTheRest`, `noDropsWhenAllowDropsIsFalse`).
- **DESIGN DECISION for f-m6-design:** (a) is drop-and-flag on by default for the nightly batch? (b) what happens to deferred corners downstream — see §5 of the infeasible-vertices doc (dedicated long-loop van / relaxed SLA / satellite hub / M3 territory reshape). **User's current call (2026-06-14): leave this for later, handle deferred corners by manual ops effort once other modules land.**
- [ ] ported to `f-m6-design`
- [ ] default-on decision + deferred-corner downstream handling recorded

### 9. Persist + expose deferred vertices (CARRY OVER — schema + API) ✅
- **Migration `V6_12__add_route_plan_deferred_vertices.sql`:** `ALTER TABLE route_plan ADD COLUMN IF NOT EXISTS deferred_vertex_ids text` (JSON array of vertex UUIDs; nullable). **Applied out-of-order on the Singapore cloud DB → v6.12 OK.** `RoutePlan.deferredVertexIds` entity field (updatable=false).
- **Endpoint:** `GET /routing/plans/{cityId}/deferred-vertices?date=` (`RoutingFleetController`, now also injects `ObjectMapper`) → `VertexGeoResponse{vertexId,lat,lon}` (emits snake_case `vertex_id`), resolving coords via `gridDataAdapter.vertexCoords`. Legit M6 read API (mirrors the enriched-stops endpoint).
- **Note:** the persistence is real M6; the **UI** that consumes it is DEMO-ONLY (see D12).
- [ ] ported to `f-m6-design` (migration renumber if needed when rebasing onto main's set)
...
| D12 | **Deferred-vertices map viz** — RoutesPanel "Meeting vertices" section with **Covered (N) / Deferred (M)** toggle chips; HexMap renders green-dot covered + red-✕ deferred markers; FitToRoutes expands to include deferred when toggled (zooms to the far corners). App fetches `getDeferredVertices` (on generate **and** on saved-plan restore), derives `coveredVertices` from `routes[].markers`, holds `showVertices{covered,deferred}` (deferred ON by default). Also **decoupled `plan.notes` from the recommendation** (was "Fleet recommendation unavailable" → neutral "⚠ Note"; recommendation always shown) and made the summary show loop/cycle as a **range** across vans | `demo-ui/src/components/RoutesPanel.jsx`, `HexMap.jsx`, `App.jsx`, `api/routingApi.js` | Pure UI consuming the item-#9 endpoint. The backend column/endpoint (item #9) IS carry-over; this overlay is not. |
| D13 | **Singapore cloud DB option** — run the demo against a 2nd Render DB in Singapore (far lower RTT than Oregon: boot ~3s vs ~50s). Source `.env` then override `SPRING_DATASOURCE_URL/USERNAME/PASSWORD` (export AFTER sourcing so it wins) before `mvn spring-boot:run -pl app`. DB already has schema + 5 grids + fleet config; demand/plans re-seeded per date via the UI | `.env` (left on Oregon), launch-time env overrides | Local-dev infra choice, not code. Host `dpg-d8mrgfrsq97s739rk1r0-a.singapore-postgres.render.com/singapore1dd`. |
...
- **2026-06-14** — Cadence/coverage rework + deferred-vertices viz, all uncommitted on `demo/m6-planning`. **Item #2 ✅ FIXED** (recommendVanCount: structural bail + probe + binary search; ~8min → ~23s). **Item #7 (CARRY OVER):** per-van cadence — assembler no longer clocks the fleet to the slowest van (M5 reads per-van times into the DA queue, so uniform cadence was never required); Delhi 2→2-4 loops. **Item #8 (CARRY OVER):** drop-and-flag — solve against the cycle target with OR-Tools disjunctions, defer unreachable far corners (`routing.solver.drop-infeasible-vertices`=true); Delhi cycle-180: 15 vans/2 loops → 25 vans/4-5 loops/20 deferred. **Item #9 (CARRY OVER):** persist deferred set (migration **V6_12** `route_plan.deferred_vertex_ids`) + `GET /routing/plans/{cityId}/deferred-vertices`. **Demo-only (D12/D13):** deferred-vertices map overlay (Covered/Deferred toggles, red-✕ markers) + notes/recommendation decoupling + loop-range summary; Singapore cloud DB option. New design doc **`docs/M6/M6-INFEASIBLE-VERTICES.md`**. 29 routing tests green. **User: carry items #2/#7/#8/#9 to `f-m6-design` next session; deferred-corner downstream handling left for manual ops until later modules land.**
