...
- [x] **B1.8** Hikari pool (`maximum-pool-size`/`minimum-idle`/timeouts, env-overridable) + Tomcat thread bounds; `validate-on-migrate=true` in `application-prod.yml` (staging stays `false` for the known grid checksum drift); API-key `lastUsedAt` write throttled to ≤1/5min (was a DB write per API call on the hot auth path). **✅ Finding FIXED (Route 1):** the from-scratch migration-ordering defect (`V1_12` alter of `onboarding_requests` sorted before its `V10` create) is resolved — `V10`/`V11` renamed to `V1_11_1`/`V1_11_2` so create/phone precede the auth alters. **Verified: all 123 migrations apply cleanly from scratch in order** (78 tables). Contents unchanged → checksums unchanged. Already-migrated DBs need a one-time history relabel (`ops/ci/relabel-onboarding-migrations.sql`) run **before** the renamed migrations deploy; fresh DBs need nothing. See `ops/ci/README.md`.
...
- [x] **B2.1** refresh-token rotation + revocation; cut 8h TTL. New `refresh_tokens` store (V1_16, SHA-256 hash only, mirrors `api_keys`); `RefreshToken`/`RefreshTokenRepository`/`RefreshTokenService(+Impl)`/`RefreshTokenProperties`. Rotation on `POST /auth/refresh` (rotate → new access JWT + new refresh in the same family; reuse of an already-rotated token = theft → revoke the whole family → 401); `POST /auth/logout` revokes (idempotent). Access TTL now minutes-granular: prod `jwt.access-token-minutes=15`, staging keeps `jwt.expiry-hours=8`. Refresh issuance on by default (`godspeed.refresh.enabled`, testable on staging); prod TTL 7d. `RefreshTokenPurgeJob` (03:30 IST, expired rows only — revoked-but-unexpired kept for reuse detection). `LoginResponse` gained a trailing `refreshToken` (back-compat; demo client still reads `token`). **Concurrency-safe (review finding #1):** rotation claims the token via an atomic conditional update (`revokeIfActive` = `UPDATE … WHERE revoked_at IS NULL`) — exactly one of N concurrent `/auth/refresh` calls wins, so "used at most once" holds under races; a lost claim within a 30s grace is a benign double-submit (two tabs — no family nuke), a later replay is theft (family revoked). Unit tests `RefreshTokenServiceImplTest` (7, incl. a 12-thread concurrency test) green; full reactor `install -DexcludedGroups=e2e` green.
- [x] **B2.2** distributed tracing (Micrometer Tracing + OTel → Grafana Tempo). Deps `micrometer-tracing-bridge-otel` + `opentelemetry-exporter-otlp` in `app`. **Dormant by default** — `management.tracing.sampling.probability=0.0` on staging (no spans recorded/exported, no collector needed); prod raises it to 10% and sets `management.otlp.tracing.endpoint` via `OTLP_TRACING_ENDPOINT`. `traceId`/`spanId` land on the MDC when a span is active, so they already surface as fields in the existing JSON logs (`<mdc/>` provider) — trace↔log correlation for free. App assembles.
- [x] **B2.3** alerting rules — `ops/alerts/godspeed-alerts.yml` (3 groups, 10 rules): golden signals (5xx rate, p95 latency, payment-path 5xx), saturation (Hikari waiters, JVM heap, Tomcat threads), infra (`up`/readiness, DLQ growth, broker disconnect). App-metric rules run off `/actuator/prometheus`; `rabbitmq_*` (DLQ/broker) need a RabbitMQ exporter/CloudAMQP scrape (documented in `ops/alerts/README.md`). External observer — no app behaviour change.
- [ ] k6 load + burst test → capacity plan; set pool/threads from data
- [x] **B2.5 (scaffold + mechanism)** / [ ] **critical-path E2E (not yet)** — Playwright FE E2E (6 consoles, `e2e/playwright/` + `.github/workflows/playwright.yml`, `workflow_dispatch`) is **smoke-only** (each console loads without an error boundary) with the login→dashboard flow `test.skip`ped; Maestro (`e2e/maestro/driver-smoke.yaml`) has the task/arrival steps `optional`. **These are scaffolds, not business-flow coverage** — real per-feature E2E is pending seeded staging accounts. e2e-in-CI (`.github/workflows/e2e.yml`, Postgres 16 service, trust auth) is **`workflow_dispatch` only — available in CI, manually triggered, NOT on the PR gate**, so a merge can happen without the DB-backed suites running. **Schema build:** the e2e job now builds the full schema **from scratch with real Flyway** (flatten all modules' migrations → `flyway migrate`, recording history for the module-scoped tests) — no committed schema dump. This is possible because the from-scratch migration-ordering defect was fixed (Route 1, see B1.8) and verified (123 migrations apply clean in order). The old `ops/ci/schema-baseline.sql` hack was **deleted**. Playwright/Maestro selectors are scaffolds to deepen as seeded staging accounts land.
- [x] **B2.6** DR restore drill (`ops/dr/restore-drill.sh`) — restores a backup into a scratch DB, asserts `flyway_schema_history` + core tables (`users`/`shipments`/`refresh_tokens`), optional `--boot` starts the app + smokes the quote endpoint, prints actual RPO/RTO vs targets. Rollback preference documented (`ops/dr/README.md`): app-only bad deploy → redeploy previous image; bad migration → forward-fix (append-only) then restore as last resort. Never touches prod. Actual drill run against a real Render backup = an operator step (needs the Render backup + creds).
- [x] **B2.7** OWASP ZAP baseline + scan-flow checklist. `.github/workflows/zap-baseline.yml` (passive DAST, `workflow_dispatch` + weekly Mon 03:00 UTC, targets staging URL via `vars.STAGING_URL`) with `.zap/rules.tsv` triage — FAILs on regressions of the B1.4 security headers / B1.6 no-leak (CSP/X-Frame/nosniff/error-disclosure/XSS/SQLi), IGNOREs justified known-safe. **Automated security baseline + regression guard, NOT a penetration test** (passive scanning won't catch most authz-level flaws); human pentest remains deferred `[ENTITY]`. Scan-flow field checklist `docs/prod-readiness/SCAN-FLOW-CHECKLIST.md` — 14 scan nodes first-mile→hub→flight→hub→last-mile, each verified for (L) append-only ledger row + (S) state transition + (SLA) leg reaction, plus 7 edge cases (dup/out-of-order/wrong-node/offline-sync/…) and ledger-integrity spot checks.
