...
- [x] **B2.5** Playwright FE E2E (6 consoles, `e2e/playwright/` + `.github/workflows/playwright.yml`, `workflow_dispatch`) + driver-app Maestro (`e2e/maestro/driver-smoke.yaml`, login→queue→"I've arrived") + e2e-in-CI (`.github/workflows/e2e.yml`, Postgres 16 service, trust auth). **Blocker surfaced & documented (`ops/ci/README.md`):** the `@Tag("e2e")` suites use `ddl-auto=validate` against a *pre-existing* full schema (auth Flyway off; orders/dispatch scoped), and a from-scratch DB trips the `V1_12`-before-`V10` ordering bug — so the e2e job loads a committed `ops/ci/schema-baseline.sql` (schema + flyway history from a cleanly-migrated DB) rather than building from zero; it loads `ops/ci/schema-baseline.sql` (**now generated + verified to load into a fresh PG16** — 77 tables/117 history rows incl. `refresh_tokens`). Kept manual/off-gate on purpose (trigger to run). Playwright/Maestro selectors are scaffolds to deepen as seeded staging accounts land.
