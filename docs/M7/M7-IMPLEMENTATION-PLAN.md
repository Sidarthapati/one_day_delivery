...
5. **Flyway `V7_1…V7_9`** (`hub/db/migration`) — the §14.3 DDL: `stand`, `flight_bag`, `bag_item`, `bag_manifest`, `stand_reassignment_audit`, `inbound_receipt`, `delivery_staging`, `hub_load_snapshot`, + seed of the **physical stand pool** for all 5 hubs (`ON CONFLICT DO NOTHING`, deterministic `md5()::uuid` ids, reusing `grid.cities` UUIDs). **No `sort_plan`/`sort_plan_entry`** — stand assignment is dynamic (M7-D-001), so there is no pre-seeded directory. Indexes per §14.3.
6. **JPA entities + repositories + enums** for all tables (append-only ones immutable + `@PrePersist`, copying `grid`/`routing` style). Enums: `SortDirection`, `StandKind`, `StandStatus`, `BagStatus`, `BagItemStatus`, `ArrivalMode`, `DiscrepancyType`, `StagingStatus`. Key finders: `FlightBagRepository.findByFlightNoAndFlightDateAndDestHubAndStatus(...)` (lazy-create lookup), `StandRepository.findFreeStands(...)` (the free flight-bag pool — stands with no open bag), `BagItemRepository.findByBagId`, etc.
7. **Inbound receiving (§6, origin modes)** — `HubReceivingService`: dock scan-in for **VAN** (sort off the `AT_ORIGIN_HUB` state, `M7-D-005`) and **SELF_DROP** (`SELF_DROP_ACCEPTED`); reconciliation vs van manifest → `inbound_receipt` (+ `HUB_DISCREPANCY` on mismatch).
...
2. **Flyway `V7_10`/`V7_11`** — `V7_10__create_delivery_bag.sql` (the dest consolidation unit: `bag_kind ROUTE|DA_TERRITORY|ZONE`, `route_plan_id`/`loop_id`/`da_territory_id`/`zone_id`, `current_stand_id`, status `OPEN|SEALED|LOADED|HANDED_OVER`, counts/weight, `manifest_id` — mirror of `flight_bag`); `V7_11__alter_delivery_staging.sql` (add `delivery_bag_id`, `da_territory_id`, `route_plan_id`; status `STAGED|LOADED|ON_SHELF|COLLECTED`). **No seed** — delivery bags open dynamically (`M7-D-001`).
...
5. **`DeliveryBagService`** — the dest mirror of `BagService`: lazy `openDeliveryBag` per key (allocates a free `DELIVERY_STAGING` stand via the same `StandRepository.findFreeStands(...)` pattern, `NoFreeStandException` → overload) → `BAG_CREATED(direction=INBOUND)`; `addParcel` → `delivery_staging` + weight/count; `seal` → `bag_manifest` (load list, append-only) + `BAG_SEALED`; M6's `VAN_LOAD` drives the bag `→ LOADED`.
