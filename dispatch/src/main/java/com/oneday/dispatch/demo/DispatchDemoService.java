...
public class DispatchDemoService {
    ...
    /** One DA GPS heartbeat for the demo's periodic ping batch. */
    public record GpsPing(UUID daId, double lat, double lon) {}

    /**
     * Demo: record a batch of DA GPS heartbeats through the <b>real</b> M5 telemetry path
     * ({@link DaStatusService#updateGps}) — so {@code da_status} genuinely tracks the moving DAs
     * ({@code last_gps_lat/lon} + {@code last_heartbeat} refresh each tick), exactly as a driver phone's
     * periodic {@code POST /dispatch/da/{daId}/gps} would. The Execution map fires this every few
     * seconds with each DA's current animated position. A DA not on shift is skipped, not fatal.
     */
    public int recordGps(List<GpsPing> pings) {
        if (pings == null) return 0;
        Instant now = Instant.now();
        int n = 0;
        for (GpsPing p : pings) {
            try {
                daStatusService.updateGps(p.daId(), p.lat(), p.lon(), now);
                n++;
            } catch (RuntimeException ignore) {
                // DA not on shift / unknown id — skip this ping, keep the rest
            }
        }
        return n;
    }
