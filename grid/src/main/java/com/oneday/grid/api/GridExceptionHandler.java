...
/**
 * Uniform error envelope for the grid API (RFC-7807 ProblemDetail). Extends Spring's base so all
 ...
 */
@RestControllerAdvice(basePackages = "com.oneday.grid")
class GridExceptionHandler extends ResponseEntityExceptionHandler {

    private static final Logger log = LoggerFactory.getLogger(GridExceptionHandler.class);
    ...
    Object handleUnexpected(Exception ex) throws Exception {
        ...
        log.error("Unhandled exception in grid API", ex);
