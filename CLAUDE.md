...
| `orders` (M4) | PRs 1–9 merged + architect-review fixes applied. PR #10 (B2C PREPAID booking) implemented in working tree but not yet committed. Flyway migrations (V4_1–V4_11), JPA entities (Shipment, ShipmentStateHistory, PaymentTransaction, B2bAccount, IdempotencyKey, ShipmentRefCounter, PickupOtp, Address embedded), Spring Data repositories (including PickupOtpRepository with pessimistic-lock verify), service layer (ShipmentStateMachine, TransitionRegistry, TransitionRegistryConfigurer, TransitionContext, CustomerVisibleStateMapper, ShipmentRefService, DeliveryTypeResolver, PaymentPort, PickupOtpService), idempotency infrastructure (IdempotencyFilter, IdempotencyKeyPurgeJob, IdempotencyProperties, V4_10 fingerprint migration), OTP infrastructure (PickupOtpService/Impl with BCrypt(4), PickupOtpProperties, PickupOtpController, V4_11 migration) — all done. PR #10 working tree: B2cShipmentController (POST /api/v1/b2c/shipments, PREPAID only, hardcoded CustomerType.B2C), BookingService/BookingServiceImpl (full serviceability→pricing→payment→persist→ETA flow with Resilience4j circuit breakers + time limiters), BookingRequest DTO (pickupType, dropType, Razorpay fields; no payment_mode or customer_type field — PREPAID hardcoded), BookingResponse DTO (shipmentRef, state, stateLabel, deliveryType, pricing, eta, parcelId, labelStatus, payment), GlobalExceptionHandler, ResilienceConfig, ResilienceProperties — implemented, not yet committed. COD path (PR #11) and B2B booking (PR #12) not yet started. |
