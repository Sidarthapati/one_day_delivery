...
| `hub` (M7) | **PR1 + PR2 merged; PR3 done (branch `f-m7-design`).** 3-PR plan in `docs/M7/M7-IMPLEMENTATION-PLAN.md` (design `M7-HUB-DESIGN.md` v0.3). Origin flight bags + destination route/territory bags (dynamic stand allocation, M7-D-001/012), M6 seam (`PARCEL_SORTED_FOR_DELIVERY`), same-city, hub-collect. **PR3 (resilience/ops):** reschedule is **M9-decided, M7-executed** (M7-D-006) — `BagReassignmentService` re-points bag(s) on a single M9 `FLIGHT_REASSIGNED` event (keyed on **flight number**, covers optimisation/cancellation/capacity/prepone-overflow via `reason`; no separate FLIGHT_CANCELLED, no M7-side SLA gate / next-flight search), append-only superseding manifest, emptied source bag → CANCELLED (frees stand), emits `BAG_RESCHEDULED`; `FlightEventConsumer` on `oneday.flight.events` (autoStartup=false — M9 unbuilt) with `FLIGHT_REASSIGNED` + advisory `FLIGHT_TIME_CHANGED` (seal-window only) handlers; `POST /hub/{hubId}/bags/reassign-flight`. **Overload (§11):** `HubLoadService` rolls `hub_load_snapshot`, emits `HUB_OVERLOAD_ALERT` (= M4 advisory throttle in v1) past `standHighWaterPct`; `GET /hub/{hubId}/load`. **Console:** `GET /hub/{hubId}/bags?date=`, `POST /hub/{hubId}/parcels/{id}/resolve` (`ParcelLocatorService`). **Config:** bag cutoff = flight departure − `hubDepartureLeadMinutes` (default 300 = 5h) in `StubFlightAssignmentPort`; removed underweight/nextFlightAfter. `FlightAssignmentPort` now read-only. **Simulation deferred to a separate demo branch** (not PR3). New common: `FlightEventType.FLIGHT_REASSIGNED/FLIGHT_TIME_CHANGED`, `FlightReassignReason`, `FlightReassignedEvent`/`FlightTimeChangedEvent` (M9 contract), `BagRescheduledEvent`/`HubOverloadAlertEvent`. Hub suite 34 green; app assembles. |
...
- **Two-UI demo harness (M4→M5→M6):** the polished demo is the **React app in `demo-ui/` on `:5173`** (Vite; proxies `/api`→`:8080`, `/osrm`→OSRM), with `:8080` static as the **customer** view ("Your Bookings", handover codes). Backend demo endpoints live under `/api/demo/**` (`app/.../demo/DemoDaController` for M4/OTP/handover + `dispatch/.../demo/DispatchDemoService` for M5 board + `routing/.../demo/DemoExecutionService` for the M6 run). **`./run-demo.sh`** boots both from the **packaged jar** (torn-build-immune — see memory) + Vite; `--build` to rebuild, `--no-ui` for backend only. **Demo tabs:** Execution (M6 map + ▶ Run the day + 📊 Ops tracker), Dispatch (M5 control tower), DA Phone. **Run flow** (all **real** bookings, no synthetic): Prepare → Load shift → 🌐 Spread pickups/drops → Auto-verify pickups → Dispatch drops → ▶ Run the day (van out, collects, parks at DA; end-of-run fires `PICKED_UP→HANDED_TO_PICKUP_VAN`) → 🏭 Complete first-mile (`→AT_ORIGIN_HUB`, drives the van home) → Auto-verify deliveries. **M5 engine** (real overflow → `CRON_INFEASIBLE` deferrals): Spread pickups `count>#DAs` wraps; ⊘ mark-absent defers the DA's pickups (`DA_ABSENT`, **deletes** the queue row so Retry-deferred re-places cleanly) + `reconcile-m4` flips orphaned `PICKUP_ASSIGNED`→`PICKUP_FAILED` ("Reassigning"); Retry deferred re-attempts. `DeferredRetryJob` interval pushed to 3600s so deferrals persist through a demo. **Messaging is 100% RabbitMQ (CloudAMQP)** — the `common/.../kafka/events` package name is historical (POJOs sent over RabbitTemplate); no Kafka broker/Confluent is used.
