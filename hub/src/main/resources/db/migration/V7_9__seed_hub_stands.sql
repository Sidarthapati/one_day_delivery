...
INSERT INTO stand (id, city_id, hub_id, stand_no, zone, kind, capacity, status)
SELECT md5(c.code || ':' || s.stand_no)::uuid, c.id, c.id, s.stand_no, s.zone, s.kind, 200, 'OPEN'
FROM (VALUES
...
CROSS JOIN (VALUES
        ('A-1', 'flight',   'FLIGHT_BAG'),
        ('A-2', 'flight',   'FLIGHT_BAG'),
        ('A-3', 'flight',   'FLIGHT_BAG'),
        ('A-4', 'flight',   'FLIGHT_BAG'),
        ('A-5', 'flight',   'FLIGHT_BAG'),
        ('A-6', 'flight',   'FLIGHT_BAG'),
        ('A-7', 'flight',   'FLIGHT_BAG'),
        ('A-8', 'flight',   'FLIGHT_BAG'),
        ('D-1', 'delivery', 'DELIVERY_STAGING'),
        ('D-2', 'delivery', 'DELIVERY_STAGING')
     ) AS s(stand_no, zone, kind)
