...
import { getDaTasks, verifyPickupOtp, resendPickupOtp, verifyDeliveryOtp, resendDeliveryOtp } from '../api/daApi.js'
...
export default function DaAppView({ cityId, cityCode }) {
  ...
  async function verify(task) {
    ...
    setBusy(true)
    try {
      if (isDrop) await verifyDeliveryOtp(task.ref, code); else await verifyPickupOtp(task.ref, code)
  ...
  async function regenerate(task) {
    if (!task.ref) { setMsg(m => ({ ...m, [task.shipmentId]: { ok: false, text: 'No booking ref (synthetic task) — cannot regenerate' } })); return }
    const isDrop = task.taskType === 'DELIVERY'
    setBusy(true)
    try {
      const { otp: newOtp } = isDrop ? await resendDeliveryOtp(task.ref) : await resendPickupOtp(task.ref)
      // Pre-fill the input so the demo can verify immediately (in prod this is sent to the customer).
      setOtp(o => ({ ...o, [task.shipmentId]: newOtp }))
      setMsg(m => ({ ...m, [task.shipmentId]: { ok: true, text: `📨 New OTP generated and pre-filled: ${newOtp}` } }))
    } catch (e) {
      setMsg(m => ({ ...m, [task.shipmentId]: { ok: false, text: '✕ ' + e.message } }))
    } finally { setBusy(false) }
  }
            ...
            <div className="rounded-[2.2rem] border-[10px] border-gray-900 bg-white shadow-2xl overflow-hidden">
              ...
              <div className="p-3 bg-gray-50 overflow-y-auto" style={{ maxHeight: '68vh' }}>
                ...
                <TaskSection title="📥 Pickups — collect from sender" tasks={tasks.pickups} color="text-blue-700"
                  kind="pickup" otp={otp} setOtp={setOtp} msg={msg} onVerify={verify} onRegenerate={regenerate} busy={busy} />
                <TaskSection title="📤 Drops — deliver to receiver" tasks={tasks.deliveries} color="text-indigo-700"
                  kind="delivery" otp={otp} setOtp={setOtp} msg={msg} onVerify={verify} onRegenerate={regenerate} busy={busy} />
              </div>
            </div>
...
function TaskSection({ title, tasks = [], color, kind, otp, setOtp, msg, onVerify, onRegenerate, busy }) {
  return (
    <div className="mb-5">
        ...
        {tasks.map(t => {
          ...
          <div key={t.shipmentId} className="bg-white rounded-lg border border-gray-200 p-3 text-xs">
            ...
            {/* OTP UI shown only in the verifiable window (pickup awaiting collection / drop out-for-delivery),
                keyed off the M4 state so a collected pickup or delivered drop shows a confirmation instead. */}
            {ph.verify && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <button onClick={() => onRegenerate(t)} disabled={busy}
                  title="Generate a fresh OTP (old one expires after 10 min; max 3 regenerations)"
                  className="text-xs px-2 py-1 rounded border border-blue-600 text-blue-600 disabled:opacity-40">Regenerate OTP</button>
              </div>
            )}
