...
// After a pickup OTP verify, flip the M5 task QUEUED→IN_PROGRESS so the run + Run-source count it as
// collected (the M4-side OTP verify only sets state PICKED_UP, not the dispatch task status).
export const markPickupCollected = (shipmentId) =>
  req(`/api/demo/da/pickups/mark-collected?shipmentId=${shipmentId}`, { method: 'POST' })
