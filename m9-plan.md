...
- **Where live flight-position data will eventually come from** — an airline's own feed, or a third-party flight-tracking service? A candidate has been identified: AirLabs, a third-party flight-tracking service that offers live position (latitude/longitude), status (scheduled / en-route / landed), and other flight details, queryable by flight number or route. It's not yet confirmed as the final choice — pricing and rate limits still need checking, and it may be superseded once a real airline partner is signed. Until then, both position and status stay simulated, along with everything else that depends on real airline data (true schedules, real cutoff times, delay/cancellation notices). Once a real airline partner is on board, all of this — whether it's AirLabs or a feed from the partner directly — gets swapped in behind the same connection point, with no change to anything else described in this plan.
